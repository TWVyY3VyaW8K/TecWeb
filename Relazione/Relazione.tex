...
Il sito richiede un database MySql. Per la creazione delle tabelle utilizzate si può usare il file .sql allegato "database.sql" che genera il database con con l'utente admin inserito di default con password "admin". All'inizio nel database non sarà presente alcuna opera che potrà essere successivamente aggiunta via upload.\\
	...
	Un utente, per essere considerato autenticato, non deve solo aver creato un account ma deve anche averne eseguito l'accesso.
		...
		Un utente autenticato ha la possibilità di cambiare la descrizione di una propria immagine tramite il pulsante Edit presente quando si visualizza un' immagine.
